#!/usr/bin/env python3
"""
OpenClaw (开物) 官方版 — Linux 图形启动器
参照 Windows 版 OC-Starter.exe 功能，即开即用
"""
...
def get_node() -> str:
    """Resolve Node.js binary path (delegates to shared utility)."""
    return get_node_binary(ROOT)


def get_engine_ver() -> str:
    """Read OpenClaw engine version (delegates to shared utility)."""
...
def get_status() -> dict:
    ...
    try:
        ...
        info["api_key"] = bool(ak and not ak.startswith("YOUR"))
...
if HAS_TKINTER:
    ...
    class App:
        def __init__(self) -> None:
            ...
            _tk_ver = str(tk.TkVersion)
            if _tk_ver.startswith("2"):  # try dark title bar (macOS-only, harmless on other platforms)
                try:
        ...
        def _build(self) -> None:
            ...
            m.rowconfigure(1, weight=0)  # cards
            m.rowconfigure(2, weight=1)  # log
            m.rowconfigure(3, weight=0)  # footer

            # ── Header ──
            hf = tk.Frame(m, bg=Cl.BG)
            hf.grid(row=0, column=0, sticky="ew", padx=24, pady=(20,4))
            tk.Label(hf, text="🦞  OpenClaw", font=("Inter",22,"bold"),
                     bg=Cl.BG, fg=Cl.TEXT).pack(side=tk.LEFT)
            self.ver_lbl = tk.Label(hf, text=f"v{get_engine_ver()} · 官方 Linux 版",
                                     font=("Inter",11), bg=Cl.BG, fg=Cl.TEXT3)
            self.ver_lbl.pack(side=tk.LEFT, padx=(10,0))
            tk.Label(hf, text="", bg=Cl.BG).pack(side=tk.RIGHT)
            ...
            actions = [
                ("🌐  Web UI", "浏览器管理界面", self._webui, "#6366f1"),
                ("⚡  Gateway", "启动 AI 网关服务", self._gateway, "#22c55e"),
                ("💬  TUI 终端", "终端聊天界面", self._tui, "#f59e0b"),
                ("📊  Dashboard", "Web 控制面板", self._dashboard, "#a855f7"),
            ]
            ...
            for i,(k,t) in enumerate(items):
                self.stat_labels[k+"_dot"] = tk.Label(sf, text="○", font=("Inter",8,"bold"),
                                                       bg=Cl.CARD, fg=Cl.TEXT3)
                self.stat_labels[k+"_dot"].grid(row=0, column=i*2, padx=(16 if i>0 else 16,2), pady=10)
                self.stat_labels[k+"_txt"] = tk.Label(sf, text=f"{t}: ...", font=("Inter",11),
                                                       bg=Cl.CARD, fg=Cl.TEXT2)
                self.stat_labels[k+"_txt"].grid(row=0, column=i*2+1, sticky="w", padx=(0,0), pady=10)
            ...
            tk.Label(lh, text="📋  输出日志", font=("Inter",10,"bold"),
            ...
            self._log(f"🦞 OpenClaw v{get_engine_ver()} · 官方 Linux 版加载完成")
            self._log(f"📁 {ROOT}")
        ...
        def _sidebar(self) -> None:
            ...
            # Logo
            tk.Label(s, text="🦞", font=("Inter",24), bg=Cl.BG2, fg=Cl.ACCENT).pack(pady=(20,4))
        ...
        def _action_card(self, parent: tk.Frame, index: int, title: str, desc: str, command, color: str) -> None:
            ...
            for w in [f, title_lbl, desc_lbl, bar]:
                ...
                w.bind("<Enter>", lambda e: f.configure(bg=Cl.CARD_HOVER) or bar.configure(bg=color))
                w.bind("<Leave>", lambda e: f.configure(bg=Cl.CARD))

            # Optional: status badge inside card
        ...
        def _set_stat(self, key: str, status: str, text: str) -> None:
            ...
            if key+"_dot" in self.stat_labels:
                self.stat_labels[key+"_dot"].configure(text=dots.get(status,"○"), fg=colors.get(status,Cl.TEXT3))
                self.stat_labels[key+"_txt"].configure(text=text)

        def _refresh(self) -> None:
            def t() -> None:
                st = get_status()
                self.root.after(0, lambda: self._set_stat("node", "ok" if st["node"]!="?" else "err", f"Node.js  {st['node']}"))
                self.root.after(0, lambda: self._set_stat("engine", "ok" if st["engine"] else "err", f"引擎  v{get_engine_ver()}"))
                self.root.after(0, lambda: self._set_stat("apikey", "ok" if st["api_key"] else "warn", f"API Key  {'已配置' if st['api_key'] else '未配置'}"))
                self.root.after(0, lambda: self._set_stat("skills", "ok", f"技能  {st['skills']} 个内置"))
            ...
            def t() -> None:
                ...
                try:
                    r = subprocess.run(
                        ...
                        env={"OPENCLAW_HOME": str(CONFIG_DIR), **os.environ.copy()})
                    out = (r.stdout or "")[:600]
                    err = (r.stderr or "")[:200]
                    if out:
        ...
        def _find_terminal(self) -> tuple[str | None, list[str] | None]:
            for name,args in [
                ("ptyxis",["ptyxis","-x"]),
                ...
                if shutil.which(name): return name, args
            return None, None
        ...
        def _run_term(self, cmd: list, desc: str) -> None:
            ...
            shell_cmd = f"cd {ROOT} && ./openclaw {' '.join(cmd)}; echo; echo '--- 按 Enter 关闭 ---'; read"
            try:
                if name in ("ptyxis", "x-terminal-emulator", "xfce4-terminal"):
                    # -x takes a single string argument; use argument list to avoid shell injection
                    ...
                    # xterm, lxterminal, tilix, terminator, urxvt: -e takes remaining args
                    subprocess.Popen(tmpl + ["bash", "-c", shell_cmd])
            except (FileNotFoundError, OSError) as e:
    ...
    def _webui(self) -> None:
        def t() -> None:
            ...
            if already_running:
                self._log(f"🌐 Web UI 已在运行 → http://localhost:{WEBUI_PORT}", "ok")
    ...
    def _dashboard(self) -> None:
        self._log("🚀 打开 Dashboard...", "ok")
        webbrowser.open("http://127.0.0.1:18789/")

    def _configure(self) -> None:
        self._run_term(["configure"], "交互式配置")

    def _channels(self) -> None:
        self._run_term(["channels", "add"], "添加通道")
...
# ══════════════════════════════════════════════
def main() -> None:
    """Open the starter config page in browser directly (CLI fallback, no tkinter)."""
